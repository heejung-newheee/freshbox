...
function FridgeSection({
  ...
  const unzoned = items.filter((i) => !i.zone || !zones.includes(i.zone as string));
    ...
    <div className="bg-white border border-gray-200 rounded-2xl p-5">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          ...
          <div className="text-xs text-gray-400 mt-0.5">{items.length}개 보관 중</div>
        </div>
      </div>
            ...
            return (
              <div key={zone}>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-xs font-bold text-gray-700">{zone}칸</span>
                  <span className="text-[11px] text-gray-400">({zoneItems.length})</span>
                </div>
                <div className="bg-gray-50 rounded-xl px-3 py-2.5 min-h-11 flex flex-wrap gap-1.5 items-center">
                  {zoneItems.length === 0
                    ...
                    : zoneItems.map((item) => <ItemChip key={item.id} item={item} />)}
                </div>
              </div>
            );
          })}
          {unzoned.length > 0 && (
            <div>
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-xs font-bold text-gray-400">구역 미지정</span>
                <span className="text-[11px] text-gray-400">({unzoned.length})</span>
              </div>
              <div className="bg-gray-50 rounded-xl px-3 py-2.5 min-h-11 flex flex-wrap gap-1.5 items-center">
                {unzoned.map((item) => <ItemChip key={item.id} item={item} />)}
              </div>
            </div>
          )}
        </div>
...
export function FridgeMap({ items }: FridgeMapProps) {
  ...
  const cols = hasKimchi ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3" : "grid-cols-1 md:grid-cols-2";
